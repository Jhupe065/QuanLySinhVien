...
namespace QuanLySinhVien
{
    public partial class frmQLDiem : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            /*
            else if (txtMaSV.Text == dgrDiem1.CurrentRow.Cells[0].Value.ToString() && cboMonHoc.Text == dgrDiem1.CurrentRow.Cells[3].Value.ToString())
            {
                {
            ...
            */
            ...
            else if (reader1.Read())
            {
                ...
                cmd1.Dispose();
                // Thực hiện truy vấn
                string insert = "Insert Into tblKET_QUA(MaSV,HoTen,MaLop,MaMon,DiemThiLan1,DiemQT,DiemTongket,HanhKiem,HocKi,GhiChu)" +
                "Values('" + txtMaSV.Text + "',N'" + txtHoTen.Text + "','" + cboLop.Text + "',N'" + cboMonHoc.Text + "','" + txtDiemThi1.Text + "','" +
                 txtDiemQT.Text + "','" + txtDiemTK.Text + "',N'" + cboHanhKiem.Text + "',N'" +
        ...
        public void FillDataGridView_Diem()
        {
            ...
            //SqlCommand cmd = new SqlCommand(select, conn);
            dgrDiem1.DataSource = DAO.DataProvider.Instance.ExecuteQuery(select);
            foreach (DataGridViewColumn column in dgrDiem1.Columns)
            {
            ...
            //// Đưa ra DataGridView
            //dgrDiem1.DataSource = this.quanlydiemDataSet47.tblKET_QUA;
            //cmd.Dispose();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                string update = "Update tblKET_QUA Set HoTen=N'" + txtHoTen.Text + "',MaMon=N'" +cboMonHoc.Text + "',MaLop='" + 
                                cboLop.Text + "',DiemThiLan1='" + txtDiemThi1.Text + "',DiemQT='" +txtDiemQT.Text + "' ,DiemTongket='" + 
        ...
        private void txtDiemThi1_TextChanged(object sender, EventArgs e )
        {
            ...
            if (txtDiemThi1.Text == "")
            {
                ...
                DIEMQT = double.Parse(this.txtDiemQT.Text);
              
                //Tính điểm TK
                DIEMTK = (0.3 * DIEMQT + 0.7 * 0);
                this.txtDiemTK.Text = Convert.ToString(DIEMTK);
            }
            else if (txtDiemQT.Text == "")
            {
                this.txtDiemQT.Text = "0";
        ...
        {
            Thaotac.Export2Excel(dgrDiem1);
        }

        private int selectedRow;
        private void dgrDiem1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            selectedRow = e.RowIndex;
            if (selectedRow < 0) { return; }
            txtMaSV.Text = dgrDiem1.CurrentRow.Cells[0].Value.ToString();
            txtHoTen.Text = dgrDiem1.CurrentRow.Cells[1].Value.ToString();
            cboLop.Text = dgrDiem1.CurrentRow.Cells[2].Value.ToString();
            cboMonHoc.Text = dgrDiem1.CurrentRow.Cells[3].Value.ToString();
            txtDiemQT.Text = dgrDiem1.CurrentRow.Cells[4].Value.ToString();
            txtDiemThi1.Text = dgrDiem1.CurrentRow.Cells[5].Value.ToString();
            txtDiemTK.Text = dgrDiem1.CurrentRow.Cells[6].Value.ToString();
            cboHanhKiem.Text = dgrDiem1.CurrentRow.Cells[7].Value.ToString();
            cboHocKi.Text = dgrDiem1.CurrentRow.Cells[8].Value.ToString();
            txtGhiChu.Text = dgrDiem1.CurrentRow.Cells[9].Value.ToString();
        }

        private void dgrDiem1_FilterStringChanged(object sender, Zuby.ADGV.AdvancedDataGridView.FilterEventArgs e)
        {
            this.tblKETQUABindingSource.Filter = this.dgrDiem1.FilterString;
        }
    }
}
